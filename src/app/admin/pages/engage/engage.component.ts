...
export class EngageComponent implements OnInit {
  ...
  constructor(
    ...
    this.messageSub = this._msgModal.getMessage().subscribe(message => {
      if (message.message && message.page === 'engage') {
        let filter = null; 
  ...
  getEngageData(filters?) {
    const unit = filters.unit === 0 ? null : filters.unit;
    const filter = { idOrgUnit: unit }
    this._app.getEngageData(filter).subscribe(
      response => {
        this.engageData = response;
        this.mountEngageChart(this.engageData.groupCountList);
        this.getEngageList(filter)
  ...
  getEngageList(filter) {
    this.engageData.groupCountList.map((item, index) => {
      let filters = {};
      if(item.filterDateOver) {
        filters['dateOver'] = item.filterDateOver;
      } else {
        filters['dateFrom'] = item.filterDateFrom;
        filters['dateTo'] = item.filterDateTo
      }
      this._app.getWorkplaceList({ ...filter, ...filters }).subscribe(
        response => {
          this.engageList[index] = {...response, type: item.label, filter: { ...filter, ...filters }};
          console.log(response)
        },
        error => {
          console.log(error);
